...
function applyCachingOptimization(expression) {
  ...
  if (effectMatches) {
    ...
    for (var i = 0; i < effectMatches.length; i++) {
      var effectMatch = effectMatches[i].match(/ctrlLayer\.effect\(["']([^"']+)["']\)/);
    ...
    for (var effectName in uniqueEffects) {
      if (uniqueEffects.hasOwnProperty(effectName)) {
        var varName = effectName.replace(/[^a-zA-Z0-9]/g, "_").toLowerCase() + "Effect";
        effectCachingCode += "var " + varName + " = ctrlLayer.effect(\"" + effectName + "\");\n";
        
        // Replace direct effect references with cached variable
        optimized = optimized.replace(
          new RegExp("ctrlLayer\\.effect\\([\"']" + effectName + "[\"']\\)", "g"),
          varName
        );
      }
    }
  ...
  if (optimized.indexOf("thisComp.") !== -1) {
    ...
    if (optimized.indexOf("thisComp.frameDuration") !== -1) {
      ...
      optimized = optimized.replace(/thisComp\.frameDuration/g, "frameDuration");
    }
...
`;
  ...
  if (optimized.indexOf("wiggleEnabled") !== -1 && optimized.indexOf("if (!wiggleEnabled)") === -1) {
...
`;
    ...
    if (wiggleInsertPoint !== -1) {
      ...
      var wiggleOptimized = 
        optimized.substring(0, wiggleInsertPoint) + 
        wiggleEarlyExitCode;
      ...
      if (optimized.indexOf("} else {") !== -1) {
        ...
        wiggleOptimized += optimized.substring(wiggleInsertPoint, elsePos) + "\n}";
...
`;
    ...
    if (optimized.indexOf("var frameDuration = thisComp.frameDuration") !== -1) {
      posterizeCode = posterizeCode.replace("thisComp.frameDuration", "frameDuration");
    }
    ...
    if (insertPoint !== -1) {
      // Find the end of variable declarations
      while (optimized.indexOf("var ", insertPoint + 4) !== -1 && 
...
function applyJSEngineOptimizations(expression) {
  ...
  if (optimized.indexOf("with(") !== -1) {
    optimized = optimized.replace(/with\s*\(([^)]+)\)\s*{([^}]+)}/g, function(match, obj, code) {
      return "var obj = " + obj + ";\n" + code.replace(/this\./g, "obj.");
    });
  }
  
  // Optimize loops
  optimized = optimized.replace(/for\s*\(var i=0;\s*i<([^;]+);\s*i\+\+\)/g, "for (var i=0, len=$1; i<len; i++)");
...
function getDefaultValueForExpression(expression) {
  if (expression.indexOf("[100, 100]") !== -1) {
    ...
    return "100";
  } else if (expression.indexOf("rotation") !== -1 || expression.indexOf("Rotation") !== -1) {
    return "0";
  } else if (expression.indexOf("scale") !== -1 || expression.indexOf("Scale") !== -1) {
    return "[100, 100]";
  } else if (expression.indexOf("position") !== -1 || expression.indexOf("Position") !== -1) {
    return "[0, 0, 0]";
  } else {
    return "0";
  }
...
function optimizeMemoryUsage(expression) {
  ...
  if (optimized.indexOf("new Array(") !== -1) {
    optimized = optimized.replace(/new Array\(([^)]+)\)/g, function(match, size) {
  ...
  if (optimized.indexOf("function(") !== -1) {
    optimized = optimized.replace(/function\s*\(([^)]*)\)\s*{([^}]*)}/g, function(match, params, body) {
      ...
      var usedVars = [];
      var bodyWithoutStrings = body.replace(/"[^"]*"/g, "").replace(/'[^']*'/g, "");
      ...
      while ((idMatch = identifierRegex.exec(bodyWithoutStrings)) !== null) {
        ...
        if (vars.indexOf(id) === -1 && 
            ...
            params.indexOf(id) === -1 && 
            ["if", "else", "for", "while", "return", "var", "function", "true", "false", "null", "undefined"].indexOf(id) === -1) {
          usedVars.push(id);
        }
      }
...
function optimizeForRenderQueue(comp) {
  try {
    ...
    if (!controlLayer) {
      errorHandler.logError("Control layer not found", "optimizeForRenderQueue", DEBUG_LEVELS.ERROR);
      return false;
    }
    ...
    var optimizationLevel = renderOptEffect.addProperty("ADBE Dropdown Control");
    ...
    var useMultiFrameRendering = renderOptEffect.addProperty("ADBE Checkbox Control");
    ...
    renderQueueItem.comment = "TextSelector optimized for rendering. Optimization level: Medium";
    
    return true;
  } catch (error) {
    if (errorHandler) {
      errorHandler.logError(error, "optimizeForRenderQueue", DEBUG_LEVELS.ERROR);
    } else {
...
function applySmartExpressionEvaluation(expression) {
  ...
  if (optimized.indexOf("wiggle(") !== -1) {
    ...
    optimized = optimized.replace(/wiggle\s*\(([^)]+)\)/g, function(match, args) {
  ...
  if (optimized.indexOf("valueAtTime") !== -1) {
    ...
    optimized = optimized.replace(/valueAtTime\s*\(\s*time\s*-\s*([^)]+)\)/g, function(match, timeOffset) {
