...
function createWiggleController(controlLayer) {
  try {
    ...
    wiggleRotation.property("ADBE Slider Control-0").setPropertyParameters([-360, 360]);
    ...
    wiggleDistortion.property("ADBE Slider Control-0").setPropertyParameters([-100, 100]);
    ...
    wiggleSeed.property("ADBE Slider Control-0").setValue(Math.floor(Math.random() * 1000) + 1); // Random default 1-1000
    wiggleSeed.property("ADBE Slider Control-0").setPropertyParameters([1, 1000]);
    ...
    if (errorHandler) {
      errorHandler.logError(error, "createWiggleController", DEBUG_LEVELS.ERROR);
    } else {
...
function applyWiggleExpressions(textLayer, controlLayerName) {
  try {
    ...
    var wigglePositionExpression = generateWigglePositionExpression(controlLayerName);
    ...
    var wiggleRotationExpression = generateWiggleRotationExpression(controlLayerName);
    // Apply to text animator rotation property
    
    // Apply wiggle distortion expression
    var wiggleDistortionExpression = generateWiggleDistortionExpression(controlLayerName);
    // Apply to text animator skew property
    
    return true;
  } catch (error) {
    if (errorHandler) {
      errorHandler.logError(error, "applyWiggleExpressions", DEBUG_LEVELS.ERROR);
    } else {
