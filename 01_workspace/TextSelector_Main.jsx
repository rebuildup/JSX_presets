...
function main() {
  ...
  try {
    ...
    alert(
      "Text Selector Modular System v" +
        TEXTSELECTOR_VERSION +
        " initialized successfully!\n\n" +
        "Control layer: " +
        result.controlLayer.name +
        "\n" +
        "Modules loaded: " +
        result.modulesLoaded.join(", ")
    );
  } catch (error) {
...
function setVersionInfo(controlLayer) {
  try {
    ...
    var versionText =
      "TextSelector Modular System v" + TEXTSELECTOR_VERSION + "\n";
    versionText += "Build Date: " + TEXTSELECTOR_BUILD_DATE + "\n";
    versionText +=
      "Modules: Core, Animation, Random, Opacity, Position, Transform, Wiggle\n";
...
function getMigrationGuidance() {
  return (
    "Migration Guide from TextSelector v1.x to v2.x\n\n" +
  ...
  try {
    ...
    if (!controlLayer) {
      ...
      if (!initResult.success) {
        result.success = false;
        result.errors.push(
          "Failed to initialize TextSelector: " + initResult.error
        );
        return result;
      }
      ...
      if (isTextLayer(layer)) {
        try {
          ...
          if (applyResult.success) {
            ...
            result.errors.push(
              "Layer '" + layer.name + "': " + applyResult.error
            );
          }
        } catch (layerError) {
          result.errors.push(
            "Layer '" + layer.name + "': " + layerError.toString()
          );
        }
      }
    }
