...
function createPresetSystem(controlLayer) {
  try {
    ...
    textDocument.text = "Select a preset category and preset, then click Apply Preset";
  ...
  try {
    ...
    if (settings.animation) {
      if (settings.animation.delay !== undefined) {
        setEffectProperty(controlLayer, "Delay", "Slider", settings.animation.delay);
      }
      
      if (settings.animation.aniStyle !== undefined) {
        setEffectProperty(controlLayer, "Ani-Style", "Slider", settings.animation.aniStyle);
      }
      
      if (settings.animation.posterize !== undefined) {
        setEffectProperty(controlLayer, "Posterize(0=FPS)", "Slider", settings.animation.posterize);
      }
    }
    ...
    if (settings.opacity) {
      if (settings.opacity.opacityStyle !== undefined) {
        setEffectProperty(controlLayer, "Opacity-Style", "Slider", settings.opacity.opacityStyle);
      }
      
      if (settings.opacity.opacityManual !== undefined) {
        setEffectProperty(controlLayer, "Opacity (Manual)", "Slider", settings.opacity.opacityManual);
      }
    }
    
    // Apply position settings
    if (settings.position && settings.position.aniPosition !== undefined) {
      setEffectProperty(controlLayer, "Ani-Position", "Point", settings.position.aniPosition);
    }
    
    // Apply transform settings
    if (settings.transform) {
      if (settings.transform.scaleOn !== undefined) {
        setEffectProperty(controlLayer, "Scale : ON", "Checkbox", settings.transform.scaleOn ? 1 : 0);
      }
      
      if (settings.transform.addScale !== undefined) {
        setEffectProperty(controlLayer, "Add Scale", "Point", settings.transform.addScale);
      }
      
      if (settings.transform.rotationOn !== undefined) {
        setEffectProperty(controlLayer, "Rotation : ON", "Checkbox", settings.transform.rotationOn ? 1 : 0);
      }
      
      if (settings.transform.addRotation !== undefined) {
        setEffectProperty(controlLayer, "Add Rotation", "Slider", settings.transform.addRotation);
      }
      
      if (settings.transform.distortionOn !== undefined) {
        setEffectProperty(controlLayer, "Distortion : ON", "Checkbox", settings.transform.distortionOn ? 1 : 0);
      }
      
      if (settings.transform.addDistortion !== undefined) {
        setEffectProperty(controlLayer, "Add Distortion", "Slider", settings.transform.addDistortion);
      }
    }
    ...
    if (settings.wiggle) {
      if (settings.wiggle.wiggleAdd !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Add", "Checkbox", settings.wiggle.wiggleAdd ? 1 : 0);
      }
      
      if (settings.wiggle.flucSec !== undefined) {
        setEffectProperty(controlLayer, "Fluc/Sec", "Point", settings.wiggle.flucSec);
      }
      
      if (settings.wiggle.wigglePosition !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Position", "Point", settings.wiggle.wigglePosition);
      }
      
      if (settings.wiggle.wiggleScale !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Scale", "Point", settings.wiggle.wiggleScale);
      }
      
      if (settings.wiggle.wiggleRotation !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Rotation", "Slider", settings.wiggle.wiggleRotation);
      }
      
      if (settings.wiggle.wiggleDistortion !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Distortion", "Slider", settings.wiggle.wiggleDistortion);
      }
      
      if (settings.wiggle.wiggleSeed !== undefined) {
        setEffectProperty(controlLayer, "Wiggle Seed", "Slider", settings.wiggle.wiggleSeed);
      }
      
      if (settings.wiggle.smoothWiggle !== undefined) {
        setEffectProperty(controlLayer, "Smooth Wiggle", "Checkbox", settings.wiggle.smoothWiggle ? 1 : 0);
      }
    }
...
function saveCustomPreset(controlLayer, presetName, description) {
  try {
    ...
    if (delayEffect) {
      settings.animation.delay = delayEffect.property("ADBE Slider Control-0").value;
    }
    
    var aniStyleEffect = findEffect(controlLayer, "Ani-Style");
    if (aniStyleEffect) {
      settings.animation.aniStyle = aniStyleEffect.property("ADBE Slider Control-0").value;
    }
    
    var posterizeEffect = findEffect(controlLayer, "Posterize(0=FPS)");
    if (posterizeEffect) {
      settings.animation.posterize = posterizeEffect.property("ADBE Slider Control-0").value;
    }
    
    // Collect opacity settings
    var opacityStyleEffect = findEffect(controlLayer, "Opacity-Style");
    if (opacityStyleEffect) {
      settings.opacity.opacityStyle = opacityStyleEffect.property("ADBE Slider Control-0").value;
    }
    
    var opacityManualEffect = findEffect(controlLayer, "Opacity (Manual)");
    if (opacityManualEffect) {
      settings.opacity.opacityManual = opacityManualEffect.property("ADBE Slider Control-0").value;
    }
    ...
    if (scaleOnEffect) {
      settings.transform.scaleOn = scaleOnEffect.property("ADBE Checkbox Control-0").value === 1;
    }
    ...
    if (rotationOnEffect) {
      settings.transform.rotationOn = rotationOnEffect.property("ADBE Checkbox Control-0").value === 1;
    }
    
    var addRotationEffect = findEffect(controlLayer, "Add Rotation");
    if (addRotationEffect) {
      settings.transform.addRotation = addRotationEffect.property("ADBE Slider Control-0").value;
    }
    
    var distortionOnEffect = findEffect(controlLayer, "Distortion : ON");
    if (distortionOnEffect) {
      settings.transform.distortionOn = distortionOnEffect.property("ADBE Checkbox Control-0").value === 1;
    }
    
    var addDistortionEffect = findEffect(controlLayer, "Add Distortion");
    if (addDistortionEffect) {
      settings.transform.addDistortion = addDistortionEffect.property("ADBE Slider Control-0").value;
    }
    
    // Collect wiggle settings
    var wiggleAddEffect = findEffect(controlLayer, "Wiggle Add");
    if (wiggleAddEffect) {
      settings.wiggle.wiggleAdd = wiggleAddEffect.property("ADBE Checkbox Control-0").value === 1;
    }
    ...
    if (wiggleRotationEffect) {
      settings.wiggle.wiggleRotation = wiggleRotationEffect.property("ADBE Slider Control-0").value;
    }
    
    var wiggleDistortionEffect = findEffect(controlLayer, "Wiggle Distortion");
    if (wiggleDistortionEffect) {
      settings.wiggle.wiggleDistortion = wiggleDistortionEffect.property("ADBE Slider Control-0").value;
    }
    
    var wiggleSeedEffect = findEffect(controlLayer, "Wiggle Seed");
    if (wiggleSeedEffect) {
      settings.wiggle.wiggleSeed = wiggleSeedEffect.property("ADBE Slider Control-0").value;
    }
    
    var smoothWiggleEffect = findEffect(controlLayer, "Smooth Wiggle");
    if (smoothWiggleEffect) {
      settings.wiggle.smoothWiggle = smoothWiggleEffect.property("ADBE Checkbox Control-0").value === 1;
    }
...
function savePresetToFile(presetName, preset) {
  try {
    ...
    var presetsFolder = new Folder(Folder.userData.fullName + "/TextSelector/Presets");
    if (!presetsFolder.exists) {
      presetsFolder.create();
    }
    
    // Create preset file
    var presetFile = new File(presetsFolder.fullName + "/" + presetName.replace(/[^a-zA-Z0-9]/g, "_") + ".json");
    presetFile.open("w");
...
function createExpressionTemplateFramework(controlLayer) {
  try {
    ...
    var templateItems = ["Custom Position", "Custom Scale", "Custom Rotation", "Custom Opacity", "Custom Wiggle"];
    ...
    if (errorHandler) {
      errorHandler.logError(error, "createExpressionTemplateFramework", DEBUG_LEVELS.ERROR);
    } else {
...
function createProjectTemplateSystem(controlLayer) {
  try {
    ...
    var templateType = projectTemplatesGroup.addProperty("ADBE Dropdown Control");
    ...
    var createProject = projectTemplatesGroup.addProperty("ADBE Button Control");
    createProject.name = "Create Project";
    
    // Add Template Description text
    var templateDescription = projectTemplatesGroup.addProperty("ADBE Text Control");
    templateDescription.name = "Template Description";
    var textDocument = templateDescription.property("ADBE Text Document").value;
    textDocument.text = "Select a template type and click Create Project to generate a new project with the selected template";
    templateDescription.property("ADBE Text Document").setValue(textDocument);
    
    return true;
  } catch (error) {
    if (errorHandler) {
      errorHandler.logError(error, "createProjectTemplateSystem", DEBUG_LEVELS.ERROR);
    } else {
...
function createUpdateCheckingSystem(controlLayer) {
  try {
    ...
    textDocument.text = "Current version: " + TEXTSELECTOR_VERSION + "\nLast checked: Never";
    updateStatus.property("ADBE Text Document").setValue(textDocument);
    
    return true;
  } catch (error) {
    if (errorHandler) {
      errorHandler.logError(error, "createUpdateCheckingSystem", DEBUG_LEVELS.ERROR);
    } else {
...
function createMigrationTools(controlLayer) {
  try {
    ...
    textDocument.text = "Use these tools to migrate from TextSelector v1.x to v2.x";
...
function createInstallationSystem(controlLayer) {
  try {
    ...
    var installationInstructions = installationGroup.addProperty("ADBE Text Control");
    installationInstructions.name = "Installation Instructions";
    var textDocument = installationInstructions.property("ADBE Text Document").value;
    textDocument.text = "TextSelector Installation Instructions:\n\n" +
    ...
    installationInstructions.property("ADBE Text Document").setValue(textDocument);
    
    // Add Check Dependencies button
    var checkDependencies = installationGroup.addProperty("ADBE Button Control");
    ...
    if (errorHandler) {
      errorHandler.logError(error, "createInstallationSystem", DEBUG_LEVELS.ERROR);
    } else {
...
function createAdvancedFeatures(controlLayer) {
  try {
    ...
    if (errorHandler) {
      errorHandler.logError(error, "createAdvancedFeatures", DEBUG_LEVELS.ERROR);
    } else {
